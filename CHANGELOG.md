...
- GitHub: add `list_contents` and `get_file` actions to the GitHub tool for autonomous repo browsing and file reading.
- GitHub: improve tool description with default repo hints and proactive usage guidance so the model uses the tool without asking.
- Jira: surface default project key and issue type in tool description to prevent model from guessing wrong values.

### Fixes

- Jira: include API error details in tool error responses for better debugging.
